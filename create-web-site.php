...
if (count($argv) < 4) {
  echo
    "USAGE: " .
    $argv[0] .
    " <host name> <owner ucsc.edu e-mail> <" .
    join("|", $site_types) .
...
$site_type = trim(strtolower($argv[3]));
...
foreach($folders as $folder) {
  ...
  if (!mkdir($this_folder_name, 0755)) {
    echo
...
################################################################################
# change ownership on the whole site folder to www:www                         #
################################################################################

passthru(
  "/usr/sbin/chown -R www:www " . escapeshellarg($site_folder_name),
  $exit_code
);
...
if (file_exists($e_mail_template)) {
  ...
  mail(
    ...
    $message,
    "From: web-notifications@soe.ucsc.edu\n",
